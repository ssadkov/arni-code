...
export const platformSessionSchema = createSchema({
	[SessionConfigKey.AutoApprove]: schemaProperty<AutoApproveLevel>({
		type: 'string',
		title: localize('agentHost.sessionConfig.autoApprove', "Подтверждения"),
		...
		enumLabels: [
			localize('agentHost.sessionConfig.autoApprove.default', "Подтверждать вручную"),
			localize('agentHost.sessionConfig.autoApprove.assisted', "С оценкой риска"),
			localize('agentHost.sessionConfig.autoApprove.bypass', "Разрешить всё"),
		],
		enumDescriptions: [
			localize('agentHost.sessionConfig.autoApprove.defaultDescription', "Спрашивает, если нет подходящего правила"),
			localize('agentHost.sessionConfig.autoApprove.assistedDescription', "Оценивает риск перед запуском инструментов"),
			localize('agentHost.sessionConfig.autoApprove.bypassDescription', "Запускает инструменты без вопросов"),
		],
		default: 'default',
		sessionMutable: true,
	}),
	...
	[SessionConfigKey.Mode]: schemaProperty<SessionMode>({
		type: 'string',
		title: localize('agentHost.sessionConfig.mode', "Режим агента"),
		description: localize('agentHost.sessionConfig.modeDescription', "Как агенту выполнять этот запрос"),
		enum: ['interactive', 'plan', 'autopilot'],
		enumLabels: [
			localize('agentHost.sessionConfig.mode.interactive', "Диалог"),
			localize('agentHost.sessionConfig.mode.plan', "План"),
			localize('agentHost.sessionConfig.mode.autopilot', "Автопилот"),
		],
		enumDescriptions: [
			localize('agentHost.sessionConfig.mode.interactiveDescription', "Работаем по шагам вместе"),
			localize('agentHost.sessionConfig.mode.planDescription', "Сначала план, потом выполнение"),
			localize('agentHost.sessionConfig.mode.autopilotDescription', "Работает сам в пределах разрешений"),
		],
		default: 'interactive',
		sessionMutable: true,
	}),
